                ...
                <div key={user._id} className="cursor-pointer hover:bg-zinc-800/50 p-3 rounded-md transition-colors group">
                    <div className="flex items-start gap-3">
                        <div className="relative">
                            ...
                            <div className={`absolute bottom-0 right h-3 w-3 rounded-full border-2 border-zinc-900 bg-zinc-500`}
                            aria-hidden="true"/>
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                                <span className="font-medium text-sm text-white">{user.fullName}</span>
                                {isPlaying && <Music className="size-3.5 text-purple-500 shrink-0"/>}
                            </div>
                            {isPlaying ? (
                                <div className="mt-1">
...
const LoginPrompt = () => (
	<div className='h-full flex flex-col items-center justify-center p-6 text-center space-y-4'>
		<div className='relative'>
			<div
				className='absolute -inset-1 bg-gradient-to-tr from-purple-600 to-purple-900 rounded-full blur-lg
